...
def monty_hall_game(switch_doors: bool) -> bool:
    ...
    #Initial doors with one car and two doors
    doors = ['car', 'goat', 'goat']
    ...
    index_revealed = [i for i in range(3) if i != initial_choice and doors[i] != 'car']
    index_revealed = random.choice(index_revealed)

    # If contestant decides to switch, their final choice is the remaining door
    if switch_doors:
       final_index = [i for i in range(3) if i != initial_choice and i != index_revealed][0]
       final_choice = doors[final_index]
    else:
...
def simulate_games(num_games: int = 1000) -> Tuple[int, int]:
